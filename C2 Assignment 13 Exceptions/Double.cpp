...
namespace suchi
{
	...
	void Double::isNaN(string value)
	{
		int numOfDecimals = 0;
	...
	bool Double::recursiveNaN(string value)
	{
		int index = 0;
		...
		else
		{
			...
			return (recursiveNaN(value.substr(index + 1, end)));
	...
	void Double::equals(string value) throw(int, string)
	{
		this->isNaN(value);
		if (!(*this->nan))
		{
